...
public class CreateVerifiCodeImage {
    ...
    public static BufferedImage getVerifiCodeImage() {
        ...
        verifiCode = generateCheckCode();  //调用generateCheckCode方法（随机生成验证码）
        drawBackground(graphics);  //调用drawBackground（绘制验证码图片背景）
        drawRands(graphics, verifiCode); //调用drawRands（绘制验证码）
    ...
    private static void drawBackground(Graphics g) {
        ...
        for (int i = 0; i < 200; i++) {
            ...
            g.setColor(getRandomColor());  //调用getRandomColor（获取随机颜色）
            g.drawOval(x, y, 1, 1);
